...
# TUI-specific commands (moved out of the workspace; crate lives elsewhere now)
# All recipes below are commented out until the TUI crate is reinstated.
#[doc("Run TUI application (debug)")]
#run-tui:
#    cargo run -p connected-tui
#
#[doc("Run TUI application (release)")]
#run-tui-release:
#    cargo run -p connected-tui --release
#
#[doc("Run TUI in daemon mode")]
#run-tui-daemon:
#    cargo run -p connected-tui -- --daemon --auto-accept
#
#[doc("Build TUI binary")]
#build-tui:
#    cargo build -p connected-tui
#
#[doc("Build TUI binary (release)")]
#build-tui-release:
#    cargo build -p connected-tui --release
#
#[doc("Print connected-tui --version")]
#tui-version:
#    cargo run -p connected-tui --quiet -- --version
#
#[doc("Generate shell completions for connected-tui")]
#completions-tui:
#    #!/usr/bin/env bash
#    set -euo pipefail
#    OUT_DIR="${COMPLETIONS_OUT_DIR:-target/completions}"
#    mkdir -p "$OUT_DIR"
#    cargo run -p connected-tui --quiet -- \
#        completions bash > "$OUT_DIR/connected-tui.bash"
#    cargo run -p connected-tui --quiet -- \
#        completions zsh  > "$OUT_DIR/connected-tui.zsh"
#    cargo run -p connected-tui --quiet -- \
#        completions fish > "$OUT_DIR/connected-tui.fish"
#    echo "Wrote completions to $OUT_DIR"
#
#[doc("Install connected-tui to /usr/local/bin (Linux)")]
#install-tui: build-tui-release
#    #!/usr/bin/env bash
#    set -euo pipefail
#    BIN="target/release/connected-tui"
#    DEST="${PREFIX:-/usr/local/bin}/connected-tui"
#    echo "Installing $BIN -> $DEST"
#    install -m 0755 "$BIN" "$DEST"
#
#[doc("Install connected-tui systemd unit (requires root)")]
#install-tui-systemd: build-tui-release
#    sudo ./packaging/systemd/install.sh
#
#[doc("Build connected-tui Docker image")]
#docker-build-tui:
#    docker build -f packaging/docker/Dockerfile -t connected-tui:dev .
#
#[doc("Run connected-tui in Docker (foreground)")]
#docker-run-tui: docker-build-tui
#    docker run --rm -it --network=host \
#        -v "{{justfile_directory()}}/tui/config.example.toml:/etc/connected-tui.toml:ro" \
#        -v connected-inbox:/var/lib/connected/inbox \
#        connected-tui:dev
#
#[doc("Bring up connected-tui via docker compose")]
#docker-compose-up-tui:
#    docker compose -f packaging/docker/docker-compose.yml up -d
#
#[doc("Tear down connected-tui via docker compose")]
